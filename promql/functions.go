...
func instantValue(vals []parser.Value, out Vector, isRate bool) Vector {
	...
	if resultValue >1000{
		...
		if len(samples.Points) > 2 {
			...
			fmt.Println("irate fix : ", "previousSample ", previousSample, " lastSample",lastSample)
			if isRate && lastSample.V < previousSample.V {
				// Counter reset.
				resultValue = lastSample.V
			} else {
...
var FunctionCalls = map[string]FunctionCall{
	...
	//newland functions
	"labels_append_ternary":	funcLabelsAppendTagsByTernary,
	"labels_append_tags":		funcLabelsAppendTags,
	"labels_rename":			funcLabelsRename,
	"labels_select":			funcLabelsSelect,
	"select1":					funcSelect1,
	"select2":					funcSelect2,
	"labels_append":			funcLabelsAppend,
	"select2_join":				funcSelect2Join,
	"distinct":					funcDistinct,
	"aggregate_matrix_max":		funcAggregateMatrixMax,
	"aggregate_matrix_min":		funcAggregateMatrixMin,
	"aggregate_matrix_sum":		funcAggregateMatrixSum,
	"labels_append_value":		funcLabelsAppendValue,
